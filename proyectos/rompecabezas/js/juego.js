...
var instrucciones = ["Primer texto de instrucciones.", "Segundo texto de instrucciones."];
// Arreglo para ir guardando los movimientos que se vayan realizando
var movimientos = [];
// ID lista con instrucciones.
...
function agregarUltimaDireccionYMostrarlo(direccion) {
...
function chequearSiGano() {
    ...
    for (let i = 0; i < grilla.length; i++) {
        for (let j = 0; j < grilla[i].length; j++) {
            if (grilla[i][j] != posicionCorrecta) {
                return false;
            } else {
                posicionCorrecta++;
            }
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
    let aux = grilla[filaPos1][columnaPos1]
...
function moverEnDireccion(direccion) {
    ...
    if (posicionValida(nuevaFilaPiezaVacia, nuevaColumnaPiezaVacia)) {
        ...
        agregarUltimaDireccionYMostrarlo(direccion)
    }
}
